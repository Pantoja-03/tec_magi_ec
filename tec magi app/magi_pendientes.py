...
try:
    ...
    base = magi_conn.get_iup_leads("Pendiente en SF' or (status = 'Cargado' and remote_id = '') or valid_program = 'False' or status = 'Procesando", True)
    base = base[(base['created_at'] > (magi_conn.get_datetime() - timedelta(days=1))) & (base['created_at'] < (magi_conn.get_datetime() - timedelta(minutes=10)))]
    base = base.fillna('')
